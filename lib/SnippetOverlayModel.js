...
function preparedDelete(state, event, catalogApi) {
  ...
  if (state.confirmAction !== "delete") return transitionResult(canceledDeleteState(state, catalogApi))
  ...
  return transitionResult(next, [{ type: "WRITE_STORE", payload: serialized.value, operationId: operationId }])
}
...
function changedDraftState(state, field, value) {
  if (state.busy || state.reconcileStatus === "blocked"
      || (field !== "title" && field !== "content") || typeof value !== "string") return state
...
function addedKeywordState(state, value) {
  if (state.busy || state.reconcileStatus === "blocked" || typeof value !== "string" || !value.trim()) return state
...
function updatedKeywordState(state, index, value) {
  ...
  if (state.busy || state.reconcileStatus === "blocked" || typeof value !== "string" || target < 0
...
function removedKeywordState(state, index) {
  var target = Number(index)
  if (state.busy || state.reconcileStatus === "blocked"
      || target < 0 || target >= state.draft.keywords.length || Math.floor(target) !== target) return state
...
function submittedCreateState(state, event, catalogApi) {
  ...
  next.pendingIntent = { kind: "create", draft: copyDraft(state.draft), operationId: operationId }
...
function preparedCreate(state, event, catalogApi) {
  if (!state.busy || !state.pendingIntent || state.pendingIntent.kind !== "create"
      || !matchesPendingOperation(state, event)) return transitionResult(state)
...
function preparedEdit(state, event, catalogApi) {
  ...
  return transitionResult(next, [{ type: "WRITE_STORE", payload: updatedBytes.value, operationId: operationId }])
}
...
function completedWrite(state, catalogApi) {
  ...
  if (state.pendingIntent.kind === "delete") {
    var searched = searchState(state.pendingIntent.nextCatalog, state.pendingIntent.query, null, catalogApi)
    if (searched.mode !== "search" || searched.results.length === 0) return transitionResult(searched)
...
function editableRecordEquals(left, right) {
  return !!left && !!right
    ...
    && valuesEqual(left.keywords, right.keywords)
}

function completeRecordEquals(left, right) {
  return editableRecordEquals(left, right)
    && left.id === right.id
...
function reconciledWrite(state, event, catalogApi) {
  ...
  if (state.pendingIntent.kind === "create") return reconciledCreate(state, event.catalog, catalogApi)
  if (state.pendingIntent.kind === "edit") return reconciledEdit(state, event.catalog, catalogApi)
  if (state.pendingIntent.kind === "delete") return reconciledDelete(state, event.catalog, catalogApi)
  return transitionResult(state)
}
...
function editorTransition(state, event, catalogApi) {
  ...
  if (event.type === "SUBMIT_CREATE" && state.mode === "create") return submittedCreateState(state, event, catalogApi)
...
function createIdEvent(exitCode, output, now, operationId) {
  ...
  return { type: "CREATE_ID_GENERATED", id: id, now: timestamp, operationId: operationId }
}
...
function reconcileReadEvent(exitCode, output, operationId, catalogApi) {
  ...
  if (loaded.type === "LOAD_SUCCEEDED") {
    return { type: "RECONCILE_SUCCEEDED", operationId: operationId, catalog: loaded.catalog }
  }
  return { type: "RECONCILE_FAILED", operationId: operationId, code: loaded.code }
}
...
function unicodeCharacters(value) {
  ...
  if (typeof Intl !== "undefined" && Intl.Segmenter && typeof Array !== "undefined" && Array.from) {
    var segmented = Array.from(new Intl.Segmenter(undefined, { granularity: "grapheme" }).segment(text))
    var segments = []
    for (var i = 0; i < segmented.length; i++) segments.push(segmented[i].segment)
    return segments
  }

  var characters = text.match(/(?:[\uD800-\uDBFF][\uDC00-\uDFFF]|[^\uD800-\uDFFF])(?:\u200D(?:[\uD800-\uDBFF][\uDC00-\uDFFF]|[^\uD800-\uDFFF]))*|[\s\S]/g)
  return characters || []
}
...
function shortcutHints() {
  return "Ctrl+N Next  ·  Ctrl+P Prev  ·  Ctrl+Shift+N Create  ·  Ctrl+E Edit  ·  Ctrl+X Delete  ·  Enter Paste  ·  Ctrl+Enter Copy"
}

function resultAccessibleName(snippet) {
  if (!snippet) return ""
  var title = String(snippet.title || "")
  var keywords = snippet.keywords && snippet.keywords.length > 0
    ? snippet.keywords.join(" · ")
    : ""
  return previewText(keywords ? title + " — " + keywords : title, 100)
}
