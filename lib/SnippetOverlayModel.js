...
function transferResult(state, event, catalogApi) {
  var kind = event.kind
  ...
  var effects = []
  var marked = catalogApi.markSnippetUsed(state.catalog, snippet.id, event.now)
  if (marked.ok) {
    var serialized = catalogApi.serializeCatalog(marked.value)
    if (serialized.ok) {
      effects.push({
        type: "WRITE_STORE",
  ...
  return transitionResult(initialState(), effects)
}
...
function transition(state, event, catalogApi) {
  ...
  if (event.type === "REQUEST_TRANSFER") return transferResult(state, event, catalogApi)
  return transitionResult(state)
}
