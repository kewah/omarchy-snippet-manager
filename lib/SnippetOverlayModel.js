...
function updatedKeywordState(state, index, value) {
  ...
  if (
    ...
    typeof value !== "string" ||
    target < 0 ||
    target >= state.draft.keywords.length ||
...
function editorTransition(state, event, catalogApi) {
  ...
  if (event.type === "UPDATE_KEYWORD")
    return transitionResult(updatedKeywordState(state, event.index, event.value))
  if (event.type === "REMOVE_KEYWORD")
    return transitionResult(removedKeywordState(state, event.index))
...
function resultAccessibleName(snippet) {
  ...
  var keywords = snippet.keywords && snippet.keywords.length > 0 ? snippet.keywords.join(" · ") : ""
