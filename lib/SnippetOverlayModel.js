...
function completedCreate(state, catalogApi) {
  if (!state.pendingIntent || state.pendingIntent.kind !== "create" || !state.pendingIntent.nextCatalog) {
...
function editorTransition(state, event, catalogApi) {
  ...
  if (event.type === "SUBMIT_CREATE") return submittedCreateState(state)
  if (event.type === "CREATE_ID_GENERATED") return preparedCreate(state, event, catalogApi)
  if (event.type === "CREATE_ID_FAILED") return failedCreateIdentity(state)
  if (event.type === "WRITE_SUCCEEDED") return completedCreate(state, catalogApi)
...
function transition(state, event, catalogApi) {
  ...
  if (state.mode === "create") return editorTransition(state, event, catalogApi)
