...
var BIND_LINE = 'o.bind("SUPER + CTRL + M", "Snippets", "omarchy-shell shell toggle kewah.snippet-manager")'

function menuFailure() {
  return { ok: false, error: { code: "INVALID_MENU", message: "Menu file was invalid" } }
}

function bindFailure() {
  return { ok: false, error: { code: "INVALID_BIND", message: "Bind file was invalid" } }
}
...
if (typeof require !== "undefined" && require.main === module) {
  ...
  process.stdin.on("end", function () {
    var result = mode === "merge-menu" ? mergeMenu(chunks.join(""))
      : mode === "merge-bind" ? mergeBind(chunks.join(""))
      : { ok: false, error: { code: "INVALID_MODE", message: "Unknown merge mode" } }
    if (!result.ok) {
