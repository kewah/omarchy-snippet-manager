...
function normalizeRecord(value) {
  ...
  if (Object.prototype.hasOwnProperty.call(value, "lastUsedAt") && value.lastUsedAt !== null) {
    if (!isUtcTimestamp(value.lastUsedAt)) return null
...
function cloneRecord(record) {
  ...
  if (record.lastUsedAt) cloned.lastUsedAt = record.lastUsedAt
  return cloned
}
...
function updateSnippet(catalog, id, changes, now) {
  ...
  var updated = cloneRecord(current)
...
function markSnippetUsed(catalog, id, now) {
  var normalized = normalizeCatalog(catalog)
  if (!normalized.ok) return normalized
  if (!isUuid(id)) return validationError("Snippet ID is invalid")
  if (!isUtcTimestamp(now)) return validationError("Snippet timestamp is invalid")
  ...
  var updated = cloneRecord(normalized.value.snippets[index])
  updated.lastUsedAt = now
  var snippets = normalized.value.snippets.slice()
  snippets[index] = updated
  var usedCatalog = { schemaVersion: 1, snippets: snippets }

  if (!catalogFits(usedCatalog)) return validationError("Snippet catalog exceeds 10 MiB")
  return success(usedCatalog)
}

function compareLastUsed(left, right) {
  var leftUsed = left.snippet.lastUsedAt
  var rightUsed = right.snippet.lastUsedAt
  if (leftUsed && !rightUsed) return -1
  if (!leftUsed && rightUsed) return 1
  if (leftUsed && rightUsed && leftUsed !== rightUsed) {
    return leftUsed < rightUsed ? 1 : -1
  }
...
function searchSnippets(catalog, query) {
  ...
  for (var i = 0; i < normalized.value.snippets.length; i++) {
    ...
    for (var j = 0; j < tokens.length; j++) {
      var token = tokens[j]
      if (title.indexOf(token) < 0 && content.indexOf(token) < 0) {
        matched = false
        break
      }
    }

    if (matched) matches.push({ snippet: cloneRecord(snippet), index: i })
  }

  matches.sort(compareLastUsed)
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    markSnippetUsed: markSnippetUsed,
