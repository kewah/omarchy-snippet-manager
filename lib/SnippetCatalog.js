...
function normalizeRecord(value) {
  ...
  var keywords = normalizeKeywords(value.keywords)

  if (!title || unicodeLength(title) > 120 || !/\S/.test(value.content) || !keywords) return null
...
function validateEditableInput(input, requireAll) {
  ...
  var hasKeywords = hasEditableValue(input, "keywords")
  ...
  var keywords = hasKeywords ? normalizeKeywords(input.keywords) : (requireAll ? [] : undefined)
  ...
  if (hasKeywords && !keywords) {
    return validationError("Keywords must be text values")
  }

  return success({ title: title, keywords: keywords, content: content })
}
...
function createSnippet(catalog, input, id, now) {
  ...
  if (findSnippetIndex(normalized.value, id) >= 0) return validationError("Snippet ID already exists")
  ...
  var snippet = {
    ...
    keywords: editable.value.keywords,
...
function updateSnippet(catalog, id, changes, now) {
  ...
  var nextKeywords = editable.value.keywords === undefined ? current.keywords.slice() : editable.value.keywords
  var nextContent = editable.value.content === undefined ? current.content : editable.value.content

  if (nextTitle === current.title && nextContent === current.content && arraysEqual(nextKeywords, current.keywords)) {
...
function searchSnippets(catalog, query) {
  ...
  for (var i = 0; i < normalized.value.snippets.length; i++) {
    ...
    var keywords = snippet.keywords.join(" ").toLowerCase()
    var score = 0
    var matched = true
    ...
    for (var j = 0; j < tokens.length; j++) {
      ...
      else if (keywords.indexOf(token) >= 0) score += 2
  ...
  matches.sort(function(left, right) {
    ...
    var titleOrder = compareText(left.snippet.title.toLowerCase(), right.snippet.title.toLowerCase())
