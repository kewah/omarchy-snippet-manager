...
test("searchSnippets requires every query token to match across searchable fields", () => {
  const source = catalog([
    record(1, { title: "Support reply", keywords: ["customer"], content: "Hello there" }),
    record(2, { title: "Support reply", keywords: ["internal"], content: "Escalate only" }),
    record(3, { title: "Greeting", keywords: ["customer"], content: "Hello there" })
  ])

  const result = SnippetCatalog.searchSnippets(source, "support customer hello")

  assert.equal(result.ok, true)
  assert.deepEqual(result.value.map((snippet) => snippet.id), [record(1).id])
})

test("searchSnippets is case-insensitive and tokenizes runs of whitespace", () => {
  const source = catalog([
    record(1, { title: "Résumé Reply", keywords: ["CAFÉ"], content: "À bientôt" })
  ])
...
test("searchSnippets ranks title matches above keywords above content", () => {
  ...
  assert.equal(result.ok, true)
  assert.deepEqual(result.value.map((snippet) => snippet.id), [
    record(3).id,
    record(2).id,
    record(1).id
  ])
...
test("searchSnippets counts only the highest-weight field for each token", () => {
  const source = catalog([
    record(1, { title: "Target zebra", keywords: ["target"], content: "target" }),
  ...
  assert.equal(result.ok, true)
  assert.deepEqual(result.value.map((snippet) => snippet.id), [record(2).id, record(1).id])
})
...
test("searchSnippets tie-breaks equal titles by exact stored ID", () => {
  ...
  assert.equal(result.ok, true)
  assert.deepEqual(result.value.map((snippet) => snippet.id), [uppercaseId, lowercaseId])
})
...
test("searchSnippets completes a representative 499-record query within 50 ms", () => {
  const snippets = Array.from({ length: 499 }, (_, index) => record(index, {
